...
class Roles(Cog):
	...
	async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
		...
		if payload.message_id == 952489920184873030: # pronoun roles
			match payload.emoji.name:
				...
				# 🧑 = They/Them
				case '👨':
					role = guild.get_role(ROLES['PRONOUN_HE'])
				case '👩':
					role = guild.get_role(ROLES['PRONOUN_SHE'])
				case '🧑':
					role = guild.get_role(ROLES['PRONOUN_THEY'])
		if payload.message_id == 952510538552868884: # ping roles
			match payload.emoji.name:
				...
				# 🎮 = Gaming Ping
				case '📢':
					role = guild.get_role(ROLES['PING_ANNOUNCEMENT'])
				case '🎞️':
					role = guild.get_role(ROLES['PING_CULT'])
				case '🎙️':
					role = guild.get_role(ROLES['PING_VC'])
				case '🎮':
					role = guild.get_role(ROLES['PING_GAMING'])
		if role:
	...
	async def color(
		...
		color: str = "COLOR_" + name.replace(" ", "_").upper()
		if ROLES[color] == 726069856528498738:
			new_color_role = ctx.guild.get_role(ROLES[color])
